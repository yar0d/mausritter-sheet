<template>
  <w-dialog v-model="showDialog" :width="500" transition="scale" :title="$t('About')" content-class="w-flex column align-start justify-center pa2">
    ...
    <div class="mt4">{{ $t('This application is copyright Daniel Coquette.') }}
      <div class="ml4 mt1"><w-icon sm bg-color="primary-dark1">mdi mdi-translate</w-icon> {{ $t('en-US') }} <w-icon>mdi mdi-arrow-right</w-icon> Yar0d</div>
      <div class="ml4"><w-icon sm bg-color="primary-dark1">mdi mdi-translate</w-icon> {{ $t('es-ES') }} <w-icon>mdi mdi-arrow-right</w-icon> CoyoteBlur</div>
      <div class="ml4"><w-icon sm bg-color="primary-dark1">mdi mdi-translate</w-icon> {{ $t('de-DE') }} <w-icon>mdi mdi-arrow-right</w-icon> Keter150</div>
      <div class="ml4"><w-icon sm bg-color="primary-dark1">mdi mdi-translate</w-icon> {{ $t('fr-FR') }} <w-icon>mdi mdi-arrow-right</w-icon> Yar0d</div>
    </div>
